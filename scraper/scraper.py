...
class AsyncScraper():
    ...
    def get_title_from_html(self, html):
        "Override in subclasses that have this field"
        return None
    ...
    async def __aenter__(self):
        """
        Async context manager entry. Launches a Playwright Chromium browser
        used by subclasses to fetch pages.
        
        Returns:
            Self: the scraper instance, ready for use inside an `async with` block.
        """
    ...
    async def __aexit__(self, exc_type, exc, tb):
        """
        Async context manager exit. Closes the Playwright browser and stops 
        the Playwright runtime.
        
        Args:
            exc_type (type[BaseException] | None): exception class if raised inside the block, else None.
            exc (BaseException | None): exception instance if raised, else None.
            tb (TracebackType | None): traceback if an exception was raised, else None.
        """
    ...
    async def get_html(self, url):
        """
        Fetch fully-rendered HTML from a URL using the Playwright browser.
        
        Opens a new browser page, navigates to the URL, and waits for the 
        network to become idle before extracting the page's content. The 
        network-idle wait ensures JavaScript-rendered content has finished 
        loading before HTML is captured.
        
        If an error occurs at any step, it is caught, logged, and None is 
        returned instead of raising (allows the scraper to continue).
        
        Args:
            url (str): The URL to fetch HTML from.
            
        Returns:
            str | None: The fully-rendered HTML if successful, None if an 
                        error occurred during navigation or extraction.
        """
        try:
